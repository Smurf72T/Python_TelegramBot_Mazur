...
2. Откройте `.env` и добавьте:
   ...
   - `ADMIN_PASSWORD` — пароль суперпользователя админки (создаётся автоматически при старте Django)

> ⚠️ **Безопасность:** обязательно установите уникальные значения для `DB_PASSWORD`, `SECRET_KEY`, `ADMIN_PASSWORD` и `TELEGRAM_BOT_TOKEN`. Никогда не используйте дефолтные значения в production!
